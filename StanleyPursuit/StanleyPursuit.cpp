...
void StanleyPursuit::setTarget(float x, float y)
{
    ...
    this->STrajectory = sqrt((this->Yf - this->Y0)*(this->Yf - this->Y0)+(this->Xf - this->X0)*(this->Xf - this->X0));
    this->sinTheta = (this->Yf - this->Y0)/(this->STrajectory);
    this->cosTheta = (this->Xf - this->X0)/(this->STrajectory);

    this->theta = atan(this->m);
    if (this->Xf < this->X0){
        this->theta = this->theta + PI;
    }
    this->di = this->sinTheta;
    this->dj = this->cosTheta;
